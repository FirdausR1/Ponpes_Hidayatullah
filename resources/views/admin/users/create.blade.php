    ...
    <div class="flex items-center justify-between rounded-2xl border border-gray-200 bg-white p-6 shadow-theme-xs">
        <div>
            <div class="flex items-center gap-2 mb-1">
                ...
                <span class="text-xs font-medium text-gray-500">Tambah Akun Baru</span>
            </div>
            <h1 class="text-xl font-bold text-gray-900">Tambah Pengguna Baru</h1>
            <p class="text-xs text-gray-500 mt-0.5">Daftarkan akun pengelola sistem dengan peran Superadmin atau Admin.</p>
        </div>
    ...
    <div class="rounded-2xl border border-gray-200 bg-white p-6 sm:p-8 shadow-theme-xs">
        <form method="POST" action="{{ route('admin.users.store') }}" class="space-y-5">
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ old('name') }}" required placeholder="Contoh: Ustadz Ahmad Fauzi, S.Pd.I" class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-xs sm:text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-500 focus:outline-none focus:ring-3 focus:ring-brand-500/10 transition">
            </div>
            ...
            <div>
                ...
                <input type="email" name="email" value="{{ old('email') }}" required placeholder="Contoh: ustadz.fauzi@hidayatullah.ponpes.id" class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-xs sm:text-sm text-gray-800 shadow-theme-xs font-mono placeholder:text-gray-400 focus:border-brand-500 focus:outline-none focus:ring-3 focus:ring-brand-500/10 transition">
            </div>
            ...
            <div>
                ...
                <div class="relative">
                    <select name="role" required class="h-11 w-full appearance-none rounded-lg border border-gray-300 bg-white px-4 pr-10 text-xs sm:text-sm text-gray-800 shadow-theme-xs focus:border-brand-500 focus:outline-none focus:ring-3 focus:ring-brand-500/10 transition">
                        <option value="bendahara" {{ old('role') === 'bendahara' ? 'selected' : '' }}>Bendahara (Hanya Kelola Data Santri &amp; Manajemen Pembayaran / Keuangan)</option>
                        <option value="admin" {{ old('role') === 'admin' ? 'selected' : '' }}>Admin (Kelola Santri, PSB, CBT, &amp; Berita; Tidak dapat ubah pengguna)</option>
                        <option value="superadmin" {{ old('role') === 'superadmin' ? 'selected' : '' }}>Superadmin (Akses Penuh Seluruh Menu &amp; Kelola Pengguna)</option>
                    </select>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-3 border-t border-gray-100">
                <div>
                    ...
                    <input type="password" name="password" required minlength="6" placeholder="Minimal 6 karakter" class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-xs sm:text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-500 focus:outline-none focus:ring-3 focus:ring-brand-500/10 transition">
                </div>
                <div>
                    <label class="mb-1.5 block text-xs sm:text-sm font-medium text-gray-700">
                        Konfirmasi Kata Sandi <span class="text-rose-500">*</span>
                    </label>
                    <input type="password" name="password_confirmation" required minlength="6" placeholder="Ulangi kata sandi" class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-xs sm:text-sm text-gray-800 shadow-theme-xs placeholder:text-gray-400 focus:border-brand-500 focus:outline-none focus:ring-3 focus:ring-brand-500/10 transition">
                </div>
            </div>
