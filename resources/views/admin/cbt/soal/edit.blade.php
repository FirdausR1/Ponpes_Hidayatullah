...
<div class="space-y-6 max-w-4xl mx-auto">
    ...
    <!-- Form Container -->
    <form action="{{ route('admin.cbt.soal.update', $question->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="ta-card p-6 space-y-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    ...
                    <div class="flex gap-2">
                        <select id="kategoriSelect" onchange="applyKategori(this.value)" class="w-1/2 px-3 py-2 text-sm bg-slate-50 border border-slate-200 rounded-lg focus:bg-white focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 outline-none">
                            ...
                            @foreach($categories as $cat)
                                <option value="{{ $cat }}" {{ $question->kategori === $cat ? 'selected' : '' }}>{{ $cat }}</option>
                            @endforeach
                            <option value="custom">+ Kategori Lainnya...</option>
                        </select>
                        <input type="text" name="kategori" id="kategoriInput" value="{{ old('kategori', $question->kategori) }}" required placeholder="Contoh: Matematika, Bahasa Arab"
                ...
                <div>
                    ...
                    <input type="number" name="bobot" value="{{ old('bobot', $question->bobot) }}" min="1" max="100" required
            ...
            <div class="flex flex-wrap items-center gap-6 p-3 bg-slate-50 border border-slate-200 rounded-lg text-xs">
                <label class="flex items-center gap-2 cursor-pointer select-none">
                    <input type="checkbox" name="is_math" id="isMathToggle" value="1" {{ old('is_math', $question->is_math) ? 'checked' : '' }} onchange="updatePreviews()" class="rounded text-brand-600 focus:ring-brand-500 w-4 h-4">
                    <span class="font-semibold text-slate-700">Soal Mengandung Rumus Matematika (KaTeX)</span>
                </label>

                <label class="flex items-center gap-2 cursor-pointer select-none">
                    <input type="checkbox" name="is_arabic" id="isArabicToggle" value="1" {{ old('is_arabic', $question->is_arabic) ? 'checked' : '' }} onchange="toggleArabicMode(this.checked)" class="rounded text-emerald-600 focus:ring-emerald-500 w-4 h-4">
                ...
                <label class="flex items-center gap-2 cursor-pointer select-none ml-auto">
                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', $question->is_active) ? 'checked' : '' }} class="rounded text-brand-600 focus:ring-brand-500 w-4 h-4">
            ...
            <div>
                ...
                <textarea name="soal" id="soalTextarea" rows="4" required oninput="updatePreviews()"
                          ...
                          class="w-full px-4 py-3 text-sm bg-slate-50 border border-slate-200 rounded-lg focus:bg-white focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 outline-none transition {{ $question->is_arabic ? 'arabic-input' : '' }}">{{ old('soal', $question->soal) }}</textarea>
            </div>
            ...
            <div class="pt-2">
                <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5 flex items-center justify-between">
                    ...
                    <span class="text-[11px] font-normal text-slate-400">Diagram, kurva, bentuk geometri, atau ayat teks bergambar</span>
                </label>

                <!-- Current Image if Exists -->
                @if(!empty($question->gambar))
                    <div id="currentImageContainer" class="p-3 bg-slate-50 border border-slate-200 rounded-xl mb-3 flex items-center justify-between gap-4">
                        <div class="flex items-center gap-3">
                            <img id="currentImageThumb" src="{{ asset($question->gambar) }}" alt="Gambar Soal Saat Ini" class="h-16 w-24 object-contain rounded border border-slate-200 bg-white">
                            <div>
                                <span class="text-xs font-bold text-slate-800 block">Gambar Soal Saat Ini</span>
                                <span class="text-[11px] text-slate-500 block">{{ basename($question->gambar) }}</span>
                            </div>
                        </div>
                        <label class="flex items-center gap-2 px-3 py-1.5 rounded-lg border border-rose-200 bg-rose-50 text-rose-700 text-xs font-semibold cursor-pointer hover:bg-rose-100 transition">
                            <input type="checkbox" name="hapus_gambar" value="1" onchange="toggleHapusGambar(this.checked)" class="rounded text-rose-600 focus:ring-rose-500 w-4 h-4">
                ...
                <div class="flex flex-col sm:flex-row items-start sm:items-center gap-4 p-3.5 bg-slate-50 border border-slate-200 rounded-xl">
                    ...
                    <span class="text-[11px] text-slate-400 whitespace-nowrap">{{ !empty($question->gambar) ? 'Pilih file jika ingin mengganti gambar' : 'Format: JPG, PNG, WEBP (Maks 2MB)' }}</span>
                </div>

                <!-- Preview Gambar Baru yang Diunggah -->
                <div id="imagePreviewContainer" class="hidden mt-3 relative inline-block">
                    <img id="imagePreview" src="#" alt="Preview Gambar Baru" class="max-h-48 max-w-full rounded-lg border border-slate-200 shadow-xs object-contain bg-white p-1">
                    <button type="button" onclick="removeImageUpload()" class="absolute -top-2 -right-2 bg-rose-600 text-white rounded-full p-1 shadow hover:bg-rose-700 transition" title="Batal unggah gambar baru">
                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                    </button>
                    <span class="text-[11px] text-emerald-600 font-semibold block mt-1">Gambar baru terpilih dan siap disimpan</span>
                </div>
            </div>
            ...
            <div class="p-4 bg-slate-50/80 border border-dashed border-slate-300 rounded-xl space-y-2">
                ...
                <div id="soalLivePreview" class="text-slate-800 text-sm md:text-base leading-relaxed p-3 bg-white rounded-lg border border-slate-200 min-h-[56px] space-y-3 {{ $question->is_arabic ? 'arabic-input' : '' }}">
                    <div>{!! nl2br(e($question->soal)) !!}</div>
                    @if(!empty($question->gambar))
                        <div id="previewExistingImgWrap">
                            <img src="{{ asset($question->gambar) }}" class="max-h-48 max-w-full rounded-lg border border-slate-200 shadow-xs object-contain my-2">
                        </div>
                    @endif
                </div>
            </div>
            ...
            <div class="pt-4 border-t border-slate-200">
                ...
                <textarea name="pembahasan" rows="2" placeholder="Catatan solusi atau cara pengerjaan untuk arsip guru penguji..."
                          class="w-full px-4 py-2 text-sm bg-slate-50 border border-slate-200 rounded-lg focus:bg-white focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 outline-none">{{ old('pembahasan', $question->pembahasan) }}</textarea>
            </div>
...
<script>
    ...
    const existingImageSrc = "{{ !empty($question->gambar) ? asset($question->gambar) : '' }}";
    let isExistingImageDeleted = false;

    document.addEventListener('DOMContentLoaded', () => {
        const textarea = document.getElementById('soalTextarea');
        lastFocusedInput = textarea;

        // Track focus on all text inputs
    ...
    function toggleHapusGambar(checked) {
        isExistingImageDeleted = checked;
        const previewWrap = document.getElementById('previewExistingImgWrap');
        if (previewWrap) {
            previewWrap.style.display = checked ? 'none' : 'block';
        }
        updatePreviews();
    }
    ...
    function updatePreviews() {
        ...
        const hasNewImage = !document.getElementById('imagePreviewContainer').classList.contains('hidden');
        ...
        if (hasNewImage) {
            html += `<div><img src="${newImagePreview.src}" class="max-h-48 max-w-full rounded-lg border border-slate-200 shadow-xs object-contain my-2"></div>`;
        } else if (existingImageSrc && !isExistingImageDeleted) {
            html += `<div><img src="${existingImageSrc}" class="max-h-48 max-w-full rounded-lg border border-slate-200 shadow-xs object-contain my-2"></div>`;
        }
